...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if show_label:
            ...
            #print(bbox_mess,c1, (np.float32(c3[0]), np.float32(c3[1])))
            print(bbox_mess,c1, c2,'-->', localization(image,c1,c2))
    return image
...
def localization(image,c1,c2):
    ...
    if ( (xcen < xx) and (ycen < yy) ):
        return 'UL'

    elif ( (xcen > xx) and (ycen < yy) ):
        return 'UR'
